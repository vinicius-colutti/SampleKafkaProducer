package com.colutti.sampleKafkaProducer.producer;
...
public class CarProducer {
    ...
    public void send(CarDTO carDTO){
        kafkaTemplate.send(topic, carDTO).addCallback(
                success -> logger.info("Sended message: " + success.getProducerRecord().value()),
                failure -> logger.error("Message failed: " + failure.getMessage())
        );
    }
